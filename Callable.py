    ...
    if not attrs:
        ...
        if attrs[0] in functionDescript:
            ...
            for k in Classes[attrs[0]]:
                print(f'descLen = {len(functionDescript[k].strip()[:10])}')
                l.append(f'''\t{k}\t{functionDescript[k].strip()[:10] 
                if len(functionDescript[k].strip()[:10])<=10 
                else functionDescript[k].strip()[:10]+'...'}\n''' )
        else:
            l.append('【错误】参数不合法\n')
            ext = printHelp()
