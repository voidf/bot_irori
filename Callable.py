    ...
    if not attrs:
        ...
        if attrs[0] in functionDescript:
            ...
            for k in Classes[attrs[0]]:
                print(f'descLen = {len(functionDescript[k].strip()[:20])}')
                l.append(f'''\t{k}\t{functionDescript[k].strip()[:20] 
                if len(functionDescript[k].strip()[:20])<20
                else functionDescript[k].strip()[:20]+'...'}\n''' )
        else:
            l.append('【错误】参数不合法\n')
            ext = printHelp()
