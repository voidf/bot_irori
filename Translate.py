...
def hhsh(req):
    ...
    re = {'text':req}
    res = json.loads(requests.post(url,headers=head,json=re,timeout=30).text)
    try:
...
def googleTrans(req):
    ...
    try:
        ...
        trans.request('GET', url, headers={'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/81.0.4044.122 Safari/537.36'})
        response = trans.getresponse()
        result_all = response.read().decode("utf-8")
        print(result_all)
